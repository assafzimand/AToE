...
For the higher order benchmarks we use larger networks: an MLP with 6 hidden layers of 128 neurons, combined with a random Fourier feature embedding~\cite{tancik2020fourier} of the input, with scale $2$ and embedding dimension equal to the layer width. To keep a network of this size affordable we train it in single precision. As suggested by recent literature, we compared a longer Adam phase followed by L-BFGS against a single run of the SOAP optimizer~\cite{vyas2024soap,wang2025gradient}, and found SOAP to be both more effective and more stable in this regime. We therefore train with 100k SOAP iterations, with learning rate $10^{-3}$ under a schedule of linear warm-up over the first 2k iterations, then exponential decay by a factor of $0.9$ every 2k iterations, following the training recipe of~\cite{wang2023expert}.

For the higher order benchmarks, whose solutions accumulate error rapidly over a long horizon (dramatically so for the chaotic KS dynamics), the root itself is trained by the sequential time marching approach of~\cite{wight2021timeadaptive}; advancing over successive time windows, rather than fitting the full horizon at once, is the common treatment of KS in the PINN literature. The time interval is divided into equal windows, 3 for KdV and 5 for KS, and an independent network with the higher order architecture above is trained on each window in order, with the converged prediction of the preceding network at the shared time slice serving as the initial condition of the next.

Each window is trained on a slightly extended interval, reaching $5\%$ of a window length past its end (the last window is capped at the final time), while predictions and the handoff to the next window use only the original window; this places the error growth that a PINN accumulates near the end of its training interval beyond the portion of the window that is actually used, a practice we adopt from the reference implementation accompanying~\cite{wang2023expert}.

For KS the Fourier feature embedding of the root is constructed to be exactly periodic in space, which we found necessary for obtaining a reasonable root for this equation. The resulting piecewise model over the windows serves as the root $u_0$, and the remainder of the pipeline, the tree construction with time tiling, the expert training, and the global fine-tuning, proceeds unchanged.

In both regimes, all stages use full batch training with 20k residual points and 256 points each for the initial and boundary conditions; the residual set is redrawn at every first order iteration and held fixed during the quasi-Newton stages of the second order benchmarks; the higher order benchmarks redraw it at every iteration (excluding the brief L-BFGS fine-tuning stage of \S\ref{sec:smoothstep_windows}, during which it is held fixed). The loss weights are $\lambda_r = \lambda_{ic} = \lambda_{bc} = 1$ for the second order benchmarks, while the higher order ones use $\lambda_{ic} = \lambda_{bc} = 1000$ with $\lambda_r = 1$; these weights are kept unchanged across all training phases.
...
A separate tree is constructed within each tile, and the nodes of all the
tile trees then compete for the single global budget: the candidate nodes of
every tile are pooled, ranked by their geometric wavelet norms, and the
$M$-term approximation \eqref{m-term} is selected over the union, a known
method for compressing decision tree approximations of a function.
Each tile's selection is then completed to a
complete binary sub-tree within its own tree, as before. In addition, the
root of every tile is always retained, so a tile that wins no terms still
contributes a single region spanning its whole slice, and the leaves of all
tile trees are guaranteed to tile the entire space--time domain. The allocation of terms across the tiles is therefore
not prescribed in advance but emerges from the solution itself: tiles whose
slice of the domain carries more fine structure win a larger share of the
budget. The leaves of all tile trees together form the decomposition, and
the rest of the pipeline proceeds unchanged.
...
Each expert is an MLP with 3 hidden layers of 24 neurons for the second order benchmarks, and, with the random Fourier feature embedding of Section~\ref{sec:root_training}, 3 hidden layers of 45 neurons for the higher order ones. In the experiments shown in Section~\ref{sec:experiments} we set $M = 8$ for the second order PDEs and $M = 25$ for the higher order ones, with 3 time tiles for KdV and 5 for KS. Since the completion to a full binary tree adds or removes leaves relative to the selected terms, the resulting number of experts varies per problem (see Tables~\ref{tab:benchmark_results_second} and~\ref{tab:benchmark_results_high}).
...
Viewed through this lens, the tree construction procedure described
...
\centering
\includegraphics[width=\textwidth]
{images/section3_method/kdv_capacity_map_M25.png}
\caption{KdV}
\label{fig:kdv_capacity}
\end{subfigure}
\hfill
\begin{subfigure}{0.45\textwidth}
\centering
...
\caption{KS}
\label{fig:ks_capacity}
\end{subfigure}

\caption{
Effective approximation capacity induced by the adaptive decomposition relative to the subdomain volume for benchmark PDEs. Trees are constructed using the $M$-term procedure of Section~\ref{sec:tree_construction} with $M=8$ for the second order examples and, with time tiling, $M=25$ for KdV and KS.}
...
A third adjustment is specific to KS. Enforcing the physical boundary conditions directly at the expert level, through the term $\lambda_{bc}\,\mathcal{L}^{\mathrm{bc}}_j$ of \eqref{eq:leaf_local_loss}, proved unstable for this chaotic equation, for the same reason that led us to build the periodicity into the root's architecture in Section~\ref{sec:root_training}. For KS we therefore remove this term and treat the faces of $\Omega_j$ that lie on the physical boundary exactly like the interior interfaces.
...
coordinate $t$, let $\delta_{j,i} = \kappa (b_{j,i} - a_{j,i})$ denote the
overlap (transition) half-width, where $\kappa > 0$ is a configurable fraction. The one dimensional window along the $i$-th coordinate is:
\begin{equation}
...
\caption{
Compact smoothstep window functions ($C^1$, $C^2$, $C^4$ and overlap fraction $\kappa = 0.2$) for a
...
producing a single, globally consistent solution. The fine-tuning stage consists of 40k SSBroyden iterations with no Adam warm-up for the second order benchmarks, and a short L-BFGS run with learning rate $10^{-4}$ for the higher order ones. In the single precision regime this final phase is less effective: the strong Wolfe line search accepts a step only if it sufficiently improves the loss, and on the already accurate composed model it quickly stops finding such steps, so training terminates after few iterations. The same condition, however, also guarantees that fine-tuning can only improve the composition, never degrade it. We found SOAP unsuitable for this final stage. Fine-tuning uses a sampling scheme that draws a fixed fraction of the collocation points from within the overlap regions; we use an overlap sampling fraction of $0.3$, with an
overlap fraction $\kappa = 0.03$ for all benchmarks.
