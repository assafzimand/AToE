...
Each expert is an MLP with 3 hidden layers of 24 neurons for the second order benchmarks, and, with the random Fourier feature embedding of Section~\ref{sec:root_training}, 3 hidden layers of 40 neurons for KdV and of 30 neurons for KS. In the experiments shown in Section~\ref{sec:experiments} we set $M = 8$ for the second order PDEs, and for the higher order ones, using 3 time tiles, $M = 12$ for KdV with a linear allocation strategy and $M = 18$ for KS with a quadratic one.
...
\caption{
Effective approximation capacity induced by the adaptive decomposition relative to the subdomain volume for benchmark PDEs. Trees are constructed using the $M$-term procedure of Section~\ref{sec:tree_construction} with $M=8$ for the second order examples and, with time tiling, $M=12$ for KdV and $M=18$ for KS.}
