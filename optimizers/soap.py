"""SOAP optimizer - Improving and Stabilizing Shampoo using Adam.
...
class SOAP(optim.Optimizer):
    ...
    Parameters:
        ...
        lr: The learning rate to use (default: 0.003).
        betas: Adam's betas parameters (b1, b2) (default: (0.95, 0.95)).
        shampoo_beta: If >= 0, use this beta for the preconditioner moving average
            instead of betas[1] (default: -1).
        eps: Adam's epsilon for numerical stability (default: 1e-08).
        weight_decay: Weight decay coefficient (default: 0.01).
        precondition_frequency: How often to update the preconditioner (default: 10).
        max_precond_dim: Maximum dimension of the preconditioner (default: 10000).
        merge_dims: Whether to merge dimensions of the preconditioner (default: False).
        precondition_1d: Whether to precondition 1D gradients (default: False).
        normalize_grads: Whether to normalize gradients per layer (default: False).
        data_format: Data format for convolutional layers (default: 'channels_first').
        correct_bias: Whether to use bias correction in Adam (default: True).
    """
    ...
    def merge_dims(self, grad, max_precond_dim):
        """Merge dimensions of gradient tensor until product <= max_precond_dim."""
    ...
    def update_preconditioner(self, grad, state,
        ...
        """Update preconditioner matrices and eigenbases (L, R, Q_L, Q_R in the paper)."""
    ...
    def get_orthogonal_matrix(self, mat):
        """Compute eigenbases of preconditioner using torch.linalg.eigh decomposition."""
        matrix = []
        for m in mat:
            if len(m) == 0:
                matrix.append([])
    ...
    def get_orthogonal_matrix_QR(self, state, max_precond_dim=10000, merge_dims=False):
        """Compute eigenbases using one round of power iteration followed by QR decomposition."""
